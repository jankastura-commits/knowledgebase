...
async function chatAnswer(question, context) {
  const system = `Jsi užitečný asistent. Odpovídej česky a drž se poskytnutého kontextu. Pokud něco v kontextu není, řekni to.`;
  const prompt = [
    { role: 'system', content: system },
    { role: 'user', content: `Otázka:\n${question}\n\nRelevantní výňatky:\n${context}\n\nOdpověz věcně a krátce.` }
  ];
  const r = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: `Bearer ${OPENAI_API_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ model: CHAT_MODEL, messages: prompt, temperature: 0.2 })
  });
  const j = await r.json();
  if (!r.ok) throw new Error(j.error?.message || r.statusText);
...
exports.handler = async (event) => {
  try {
    ...
    const rows = raw.split(/\r?\n/).map(l => l.trim()).filter(Boolean).map(l => {
      ...
      return { emb: j.embedding, file: j.file, ref: j.ref, fileId: j.fileId };
    });
    if (!rows.length) return json(400, { error: 'Index je prázdný.' });

    // 3) embed dotazu a vyber Top-K
    const qvec = await embed(question);
    const scored = rows.map(r => ({ ...r, score: cosine(qvec, r.emb) }))
                       .sort((a,b)=>b.score-a.score)
                       .slice(0, Math.max(1, Math.min(20, topK)));

    // 4) poskládej kontext pro model
    const context = scored.map((r,i)=>`[${i+1}] (${r.file} • ${r.ref})\n${/* placeholder */''}`).join('\n');
    // (Pozn.: v indexu máme embeddingy bez původních textů – to je v pohodě; pro stručné odpovědi stačí názvy/refs.
    // Pokud chceš vracet i úryvky textu, můžeme do indexu ukládat i krátké "snippet" pole.)

    const answer = await chatAnswer(question, context);
    const citations = scored.map((r,i)=>({ ref:`${i+1}`, file:r.file }));
